...
> [!IMPORTANT]
> 本文件是 **[`.agents/AGENTS.md`](.agents/AGENTS.md)** 的完整镜像（用于让红线常驻 AI 客户端系统提示）。
> **真源在 `.agents/AGENTS.md`**：如两者不一致，以 `.agents/AGENTS.md` 为准，并请同步本镜像。
> 更完整的接入步骤见 [`.agents/README.md`](.agents/README.md) 与根目录 [README.md](README.md)。
...
6. **事件总线订阅（按 NeoForge 小版本）**：`@EventBusSubscriber` 的监听方法必须 `static`。先读取宿主精确 `neo_version`：在 **21.1.0～21.1.180** 中注解默认 `Bus.GAME`，监听 `IModBusEvent` 须显式 `bus = Bus.MOD`；从 **21.1.181 起**会自动订阅并分流到正确总线，应省略 `bus`。细节见 [event_system.md](.agents/skills/neoforge/references/event_system.md)。
...
1. **资源生成与 DataGen**：配方、掉落表、模型、标签等 JSON 须经 `DataProvider` + 门禁更新；禁止手写（`zh_cn.json` 与 metadata 除外）。目录名单数（`loot_table`、`recipe` 等）。交付/发布线标准见 [quality_bar.md](.agents/skills/neoforge/references/quality_bar.md)。
